...
def main(spark):
    ...
    # Load and sort by zipcode
    df_big = spark.read.csv('hdfs:/user/pw44_nyu_edu/peopleBig.csv', schema=schema, header=False)
    df_big_sorted = df_big.sort("zipcode")

    # Write to new sorted parquet
    df_big_sorted.write.mode('overwrite').parquet('hdfs:/user/jg8316_nyu_edu/people_big_sort.parquet')
    print(" Wrote people_big_sort.parquet (sorted by zipcode)")
