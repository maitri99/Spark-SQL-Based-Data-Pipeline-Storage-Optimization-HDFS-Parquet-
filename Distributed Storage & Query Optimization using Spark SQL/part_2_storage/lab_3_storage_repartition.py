...
def main(spark):
    ...
    # Repartition and save optimized big parquet
    df_big = spark.read.csv('hdfs:/user/pw44_nyu_edu/peopleBig.csv', schema=schema, header=False)

    # Optimization: Repartition by zipcode
    df_big_repart = df_big.repartition("zipcode")
    df_big_repart.write.mode('overwrite').parquet('hdfs:/user/jg8316_nyu_edu/people_big_repart.parquet')
    print(" Wrote people_big_repart.parquet (repartitioned by zipcode)")
