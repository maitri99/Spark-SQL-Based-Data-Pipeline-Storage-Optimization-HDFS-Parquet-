...
def main(spark):
    ...
    # Combo optimization: repartition + sortWithinPartitions + partitionBy
    df_big_opt = df_big.repartition("zipcode").sortWithinPartitions("zipcode")
    df_big_opt.write.mode('overwrite').partitionBy("zipcode").parquet('hdfs:/user/jg8316_nyu_edu/people_big_combo.parquet')
    print(" Wrote people_big_combo.parquet (combo optimized)")
